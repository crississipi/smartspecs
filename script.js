...
// Image Fetching Button Handler
if (fetchImagesBtn) {
    let statusCheckInterval = null;
    
    // Function to check status and update button
    async function checkImageFetchStatus() {
        try {
            const response = await fetch('api/fetch_images.php?action=status');
            ...
            if (data.success) {
                ...
                if (status.can_run) {
                    btn.disabled = false;
                    btn.title = `Fetch Images (${status.queries_remaining}/${status.queries_limit} queries remaining)`;
                    // Update text while keeping the image
                    const img = btn.querySelector('img');
                    const textNode = Array.from(btn.childNodes).find(n => n.nodeType === 3);
                    if (textNode) {
                        textNode.textContent = ` Fetch Images (${status.queries_remaining} left)`;
                    } else if (img && img.nextSibling) {
                        img.nextSibling.textContent = ` Fetch Images (${status.queries_remaining} left)`;
                    } else {
                        btn.appendChild(document.createTextNode(` Fetch Images (${status.queries_remaining} left)`));
                    }
                } else {
                    btn.disabled = true;
                    btn.title = 'Daily query limit reached. Please try again tomorrow.';
                    // Update text while keeping the image
                    const img = btn.querySelector('img');
                    const textNode = Array.from(btn.childNodes).find(n => n.nodeType === 3);
                    if (textNode) {
                        textNode.textContent = ' Fetch Images (Limit Reached)';
                    } else if (img && img.nextSibling) {
                        img.nextSibling.textContent = ' Fetch Images (Limit Reached)';
                    } else {
                        btn.appendChild(document.createTextNode(' Fetch Images (Limit Reached)'));
                    }
                }
                
                // Show progress if available
                if (status.progress && status.progress.last_file) {
                    btn.title += ` | Last: ${status.progress.last_file} - ${status.progress.last_component_name || 'N/A'}`;
                }
            }
        } catch (error) {
            console.error('Error checking image fetch status:', error);
        }
    }
    
    // Check status on page load
    checkImageFetchStatus();
    
    // Check status every 30 seconds
    statusCheckInterval = setInterval(checkImageFetchStatus, 30000);
    
    // Handle button click
    fetchImagesBtn.addEventListener('click', async function() {
        if (this.disabled) {
            showNotification('Daily query limit reached. Please try again tomorrow.', 'error');
            return;
        }
        
        // Confirm action
        if (!confirm('Start fetching component images? This will use your daily query quota.')) {
            return;
        }
        ...
        if (textNode) {
            textNode.textContent = ' Starting...';
        } else if (img && img.nextSibling) {
            img.nextSibling.textContent = ' Starting...';
        } else {
            this.appendChild(document.createTextNode(' Starting...'));
        }
        
        try {
            const response = await fetch('api/fetch_images.php?action=run', {
                method: 'POST'
            });
            const data = await response.json();
            
            if (data.success) {
                showNotification('Image fetching script started! It will run in the background.', 'success');
                // Update status after a delay
                setTimeout(checkImageFetchStatus, 2000);
            } else {
                showNotification(data.message || 'Failed to start image fetching script', 'error');
                checkImageFetchStatus();
            }
        } catch (error) {
            console.error('Error starting image fetch:', error);
            showNotification('Error starting image fetching script', 'error');
            checkImageFetchStatus();
        }
    });
}